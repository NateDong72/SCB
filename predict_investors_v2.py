...
df_investors_pool                    = df_detailbonds_original_filtered_sum.iloc[ :len(df_detailbonds_original_filtered_sum) , :]
...
if special:
    ...
    for each_investor in df_investors_pool['investor_name']:
        if each_investor in dict_investor_country:
            if dict_investor_country[each_investor] != ISSUER_COUNTRY:
                list_country.append(each_investor)
    
        if int(df_investor_name_count_global_freq.loc[df_investor_name_count_global_freq['investor_name'] == each_investor]['count']) < 8:
            list_count.append(each_investor)

    set_remove =  set(list_country) & set(list_count)
    
    df_investors_pool = df_investors_pool[~df_investors_pool['investor_name'].isin(set_remove)]
...
if reduce_size and len(selected_investors) > 90:
    new_size = 90 - len(investors_selected_high_local_freq)
    if new_size < 0:
        new_size = 0
    
    df_investors_first_filter = df_investors_first_filter[~df_investors_first_filter['investor_name'].isin(investors_selected_high_local_freq)]
    df_investors_first_filter = df_investors_first_filter.iloc[ :new_size , :]
...
df_investor_score_2nd_group_filtered = df_investor_score_2nd_group.loc[df_investor_score_2nd_group['investor name'].isin(set(df_detailbonds_original_filtered_sum['investor_name']))]
rest_selected_investors              = df_investor_score_2nd_group_filtered.iloc[ :100-len(selected_investors) , :]
...
#==============================================================================
    ...
    one_row = [each_investor, round(firm_size,2), round(firm_size_min,2), round(firm_size_max,2)]
...
output.columns  = ['investor_name','firm_size','min','max']
output          = output.sort_values(by='firm_size',ascending  = False)
